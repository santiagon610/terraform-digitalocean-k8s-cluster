...
| <a name="input_argocd_oidc_rbac_scopes"></a> [argocd\_oidc\_rbac\_scopes](#input\_argocd\_oidc\_rbac\_scopes) | ArgoCD RBAC claim(s) to read group membership from, as a casbin scope list string (e.g. "[groups]"). Some IdPs (e.g. Entra ID with group-to-approle assignment) emit membership under a different claim such as "roles". | `string` | `"[groups]"` | no |
| <a name="input_argocd_oidc_readonly_group"></a> [argocd\_oidc\_readonly\_group](#input\_argocd\_oidc\_readonly\_group) | OIDC readonly group for ArgoCD. See argocd\_oidc\_admin\_group for how this must be formatted per-IdP. | `string` | `"argocd-readonly"` | no |
| <a name="input_argocd_oidc_requested_scopes"></a> [argocd\_oidc\_requested\_scopes](#input\_argocd\_oidc\_requested\_scopes) | OIDC scopes to request for ArgoCD login. Defaults cover standard OIDC claims; some IdPs require additional scopes to populate group membership in the ID token. | `list(string)` | <pre>[<br/>  "openid",<br/>  "email",<br/>  "profile"<br/>]</pre> | no |
